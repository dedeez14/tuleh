...
**Penjualan terukur** (desktop ≥ 0.9.28, Android ≥ 2.27.0): barang bersatuan Kg/liter/meter/ons dijual per ukuran atau per nominal ("mangga dua puluh ribu"). Nominal diterjemahkan ke ukuran lebih dulu — pembulatan ke bawah ke kelipatan langkah satuan — sehingga uang tetap `ukuran × harga` dan cocok dengan hitungan server tanpa perubahan kontrak. Aturannya di `lib/satuan-terukur.js` & `core/utils/satuan_terukur.dart` (tabel dan pembulatan wajib sama). Rancangan lengkap: [PENJUALAN-TERUKUR.md](PENJUALAN-TERUKUR.md).
